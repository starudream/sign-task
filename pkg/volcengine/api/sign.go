...
func addSign(r *resty.Request, method, addr, path, action, version, region, service string, account config.Account) {
	...
	r.SetHeader("x-date", date)

	r.SetQueryParam("Action", action)
	...
	scopeStr := strings.Join([]string{date[:8], region, service, "request"}, "/")
	signStr := strings.Join([]string{Algorithm, date, scopeStr, util.SHA256Hex(reqStr)}, "\n")
	keyBs := util.HMAC256(util.HMAC256(util.HMAC256(util.HMAC256(account.Secret, date[:8]), region), service), "request")
...
func genHeader(header http.Header) (string, string) {
	...
	for i := 0; i < len(keys); i++ {
		...
		buf.WriteString(key)
		buf.WriteByte(':')
		buf.WriteString(strings.TrimSpace(header.Get(key)))
		buf.WriteByte('\n')
		lks[i] = key
	}
